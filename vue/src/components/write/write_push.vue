<template>
  <div>
    <van-swipe class="my-swipe" :autoplay="3000" indicator-color="white">
      <van-swipe-item @click="toFlower">飞花</van-swipe-item>
      <van-swipe-item @click="toPic">猜句</van-swipe-item>
      <van-swipe-item @click="toPoet">没想好名字</van-swipe-item>
      <van-swipe-item>4</van-swipe-item>
    </van-swipe>
    <div class="writeList">

      <div style="margin-left: 20px; margin-top: 20px; color: brown;">
        精选
      </div>
        ...
        <van-cell v-for="item in list">
          <div class="writeListItem">
            ...
            <div class="info">
              ...
              <van-button v-if="follewList.findIndex((n) => n == item.uID) != -1" type="info" color="brown" size="small"
                @click="CancelNoticeUser(item)" >
                已关注
              </van-button>
            </div>
...
export default {
  ...
  methods: {
    async onLoad() {
      const { data: res } = await this.$http.post("writes/searchWritesAll");
    ...
    toFlower() {
      this.$router.push({ path: "/flower/Feng" });
    },
    toPic() {
      this.$router.push({ path: "/gamePic" });
    },
    toPoet() {
      this.$router.push({ path: "/game_p/li" });
    },
