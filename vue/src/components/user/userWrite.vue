<template>
    <div>
        ...
        <van-sticky :offset-top="45">
            <div class="topBar">
                <van-nav-bar left-text="返回" left-arrow @click-left="onClickLeft" title="我的发帖">
        ...
        <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
            <van-cell v-for="item in list" :key="item.id" :title=item.headline :label="item.content + '\n' + item.time"
...
export default {
    ...
    methods: {
        ...
        async onLoad() {
            ...
            const { data: res } = await this.$http.post("/writes/searchWritesByUID?UID=" + UID);
