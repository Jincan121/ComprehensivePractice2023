...
public class DaoUser {
    ...
    public int insertUser(String uid,String username,String password,String email){
        String sql = "insert into User(UID,username,password,email,image,FengHuaXueYue,LibaiDuSuLiqing) values(?,?,?,?,?,?,?)";
        return jdbcTemplate.update(sql,new Object[]{uid,username,password,email,"1","0","0"});
    }
    ...
    public int modifyImage(String UID, String image){
        String sql= "update User set image="+image+" where UID="+UID;
        return jdbcTemplate.update(sql);
    }
    ...
    public String findImageByID(String uid){
        String sql="select image from User where UID= "+uid;
        return jdbcTemplate.queryForObject(sql,String.class);
    }
